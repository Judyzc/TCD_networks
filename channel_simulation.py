...
def send_w_delay_loss(sock, data, addr):
    # Send data after simulating transmission delay using a thread
    # Note: sock is now a UDP socket and addr is the destination address tuple (ip, port)
    ...
    def send_or_drop_delayed():
        if not_dropped:
            ...
            try:
                # Changed from conn.sendall() to sock.sendto() for UDP
